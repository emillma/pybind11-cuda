...
unsigned py_get_crc_cuda(long pycuvec, int len, long crc_table,
    ...
    int numBlocks = 16;
    int blockSize = 1024;
    crc_cuda<<<numBlocks, blockSize>>>(d_vec, len, d_crc_table, d_join_tables,
                                       d_res);
